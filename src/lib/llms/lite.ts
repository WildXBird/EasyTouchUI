...
export class LiteLLM {
    ...
    private continueAsk(message: string, hide: boolean, temperature?: number,role?:Role) {
    ...
    async exec(str: string, hide: boolean, temperature: number,role?:Role) {
        const data = await this.continueAsk(str, hide, temperature,role)
        return data
    }
    /** 中断正在进行的生成 */
    async discontinue() {
